...
class AkashWorker(worker.Worker):
    ...
    async def generate_text(self, context : context.Context) -> context.Text:
        if context.body.get("model") not in self.available_models:
            raise error.WorkerUnsupportedError(f"Model {context.body['model']} not available")
        if context.body.get("model") == "AkashGen":
            raise error.WorkerUnsupportedError(f"Model {context.body['model']} for image generation only")
